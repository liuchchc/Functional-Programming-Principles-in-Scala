...
trait GameDef {
  ...
  case class Pos(x: Int, y: Int) {
    ...
    override def hashCode(): Int = x + 100*y
    
    override def equals(that: Any): Boolean = {
      def strictEqual(other: Pos) = this.x == other.x && this.y == other.y
      that match {
        case a: AnyRef if this eq a => true
        case p: Pos => (p canEqual this) && strictEqual(p)
        case _ => false
      }
    }
  }
  ...
  case class Block(b1: Pos, b2: Pos) {
    ...
    def left = if (isStanding)         dy(-2, -1) // 站立，左移，倒下，X不变，B1移动2，B2移动1
               else if (b1.x == b2.x)  dy(-1, -2) // 躺着，平行Y，站起来，X不变，B1移动1，B2移动2
               else                    dy(-1, -1) // 躺着，平行X，滚动，X不变，B1-B2移动1
